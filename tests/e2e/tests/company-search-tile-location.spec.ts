 ...
 * PS_TWO_COMPANY_SEARCH_LOCATION switch ("Enable Company Search In Address
...
function setCompanySearchInAddressArea(enabled: boolean): void {
  ...
  execFileSync("docker", [
    ...
    "memory_limit=512M",
    "-r",
    `require "/var/www/html/config/config.inc.php"; Configuration::updateValue("PS_TWO_COMPANY_SEARCH_LOCATION", ${enabled ? 1 : 0});`
  ]);
...
test.describe("TWO-25326 §7.1 company-search location", () => {
  ...
  test("payment tile: address-area company field stays visible and plain (never hidden), tile mount renders and is interactive", async ({
    ...
    // The tile mount: rendered only because PS_TWO_COMPANY_SEARCH_LOCATION=0,
