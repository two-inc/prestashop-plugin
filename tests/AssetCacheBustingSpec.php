...
final class AssetCacheBustingSpec
{
    ...
    private static function testCheckoutHookUsesCleanPathAndVersionParamForEveryRegisteredAsset(): void
    {
        ...
        // at all, so there is no first paint to beat and no reason to pay for a
        // render-blocking request on every checkout page. Pinned as an empty set
        // rather than deleted, so re-introducing a head-positioned asset is a
        // deliberate, visible decision rather than a quiet one.
        $headCalls = array();
        ...
        TinyAssert::same(
            array(),
            $headCalls,
            'no front asset may be registered head-side: the surcharge sync no longer reloads the payment '
            . 'step, so nothing needs to run before the payment markup is parsed'
        );
    }
