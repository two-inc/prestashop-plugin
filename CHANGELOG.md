...
- **"No tax" is no longer offered as a surcharge tax treatment, and the field is now called "Surcharge Tax Treatment"** (TWO-25279, umbrella TWO-24739)
  ...
  - **A shop that already stores `No tax` keeps it in the dropdown**, and that carve-out is load-bearing rather than cosmetic. A `<select>` cannot render a value that is absent from its options, so the browser would post the leading placeholder on the next *unrelated* Payment-settings save, and `validTwoSurchargeFormValues()` rejects a blank treatment while surcharges are enabled — which would lock the shop out of saving **any** Payment setting, not just this one. The stored value is read through `getTwoSurchargeTaxRulesGroupFormDefault()`, never `getTwoSurchargeTaxRulesGroupId()`: the latter collapses unset/blank/garbage to `0` and would re-offer the option to every shop that has not chosen yet
  - Server-side validation still **accepts** `0` for the same reason. The suppression lives in the option list, which is what stops anyone newly choosing it
  ...
  - **No migration**, and therefore no new upgrade script and no schema change: `PS_TWO_SURCHARGE_TAX_RULES_GROUP` keeps its name and its stored values, and no shop's checkout tax changes. Group `0` resolves `0.0` at runtime exactly as before
  - `translations/es.php` carries no entry for any of the surcharge configuration strings, so no translation is orphaned by the new source hashes. That gap predates this change and is unaffected by it
...
- **Reverted: the zero-cap "overcharge" guard withheld Two from every buyer on affected shops** (TWO-25276, reverting part of TWO-25269, umbrella TWO-24739)
  ...
  - **No rounding was changed.** The pricing API rejects values below the minor unit, so an unrounded figure is an HTTP 400, not a more precise cap. Every `round($converted, 2)` stays
