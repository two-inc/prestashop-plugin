...
### Changed
- **The in-field company-search spinner is drawn in CSS instead of being an animated GIF** (TWO-25288, umbrella TWO-24739)
  - The spinner was `views/img/loader.gif`, set as the company input's `background-image`. That shipped a binary asset, rendered at one fixed pixel size whatever the field's type size was, and could not take a colour - so a brand overlay had no way to make it match anything. It is now twelve radial spokes drawn with a `repeating-conic-gradient` and a `radial-gradient` mask, coloured from `var(--two-spinner-color, currentColor)`, which an overlay sets once
  - **The GIF was referenced by TWO rules, and removing only one would have changed nothing visible.** Alongside the rule scoped to the company field there was a second, unscoped `.ui-autocomplete-loading` rule that matched *every* jQuery UI autocomplete on the page and forced a white background and its own padding with `!important`. Both are gone, and so is the asset - nothing else in the module referenced it
  - **The spinner had to become a real element.** A CSS-drawn spinner cannot be a `background-image`, and an `input` accepts neither `::before` nor `::after`, so a `<span>` is now inserted immediately after the company input. Visibility is still driven *entirely* by the loading class on the input, through a sibling selector, so **neither render path needed a change to arm it**: jQuery UI already sets `ui-autocomplete-loading` itself, and the custom no-jQuery-UI fallback already sets `two-company-search-loading` by hand. One CSS contract, both paths
  - The **hiding** half of that is unconditional rather than sibling-scoped, deliberately: a sibling-scoped hide would leave a span whose input had lost `two-company-search-input` matching neither rule, so it would fall back to the browser default of `display: inline` and spin forever. Hidden-unless-told-otherwise is the right default for an element whose whole job is to appear briefly
  - **Insertion happens in `setupAutocomplete()`, not in `init()`, and is idempotent by reading the DOM.** PrestaShop replaces the address form's markup on `updatedAddressForm`, which takes a previously inserted span with it, so the span has to be re-created against the replaced field - and `setupAutocomplete()` is the method that event already re-runs. Idempotence is decided by asking the input's parent what is actually there rather than by a remembered reference, because a reference can point at a node the platform has already detached, and the live instance holds no reference to a span a *destroyed* instance inserted. Both cases end in two spinners in one field
  - **The `~` combinator rather than `+`, and that distinction is load-bearing.** The fallback path inserts its dropdown container directly after the input, so on that path the spinner is *not* the input's adjacent sibling. An adjacent-sibling selector would match the container instead and the spinner would never appear on the fallback path - while every class-level assertion still passed. A test pins it: changing `~` to `+` turns two fallback-path cases red
  - **The fallback path's own insertion point is deliberately left alone.** An earlier revision re-anchored the dropdown container to sit after the spinner, on the reasoning that a block element between input and spinner would push the spinner out of the field's line box. That reasoning was wrong: the spinner is `position: absolute` against the shared parent, so it is out of flow and its rendered position does not depend on sibling order at all. The re-anchor was a no-op with a plausible-sounding rationale, and it is gone
  - **Never centred with `transform`.** The animation owns `transform`, so `translateY(-50%)` centring would be overwritten on the first frame and drop the spinner half its height out of the field. `top` with `calc` does the centring, and a test asserts that no `transform` resolves on the element
  - `destroy()` removes the element and the positioning class as well as the classes, since a class-only teardown would now leave the span behind
  - **No `@supports` fallback for the conic gradient, deliberately.** `repeating-conic-gradient` needs Safari 12.1 / Chrome 79 / Firefox 83 and silently draws nothing below that. The stylesheet already ships `inset`, whose support floor (Safari 14.1, Chrome 87) is strictly *newer*, so a fallback here would guard a browser the rest of the file has already given up on
  - **Keyframes reused, not duplicated**: the stylesheet's existing `two-spin` ("one full turn") drives it, with a comment at the definition recording that it is now shared. The eligibility overlay's own rotating-arc spinner is deliberately **untouched** - it is a different element with a different job, and its colour is still a hard-coded hex value rather than brand-colourable. Worth a follow-up, not part of this change
  - **The timing function is `linear`, and a stepped one would have shipped a spinner that never moved.** The spoke pattern has a 30deg period, so `steps(12, end)` over one turn advances in 30deg increments and maps the pattern exactly onto itself - present, correctly coloured, correctly sized, and completely motionless. The trap catches any step count that is a whole divisor of the spoke count, and it fails silently. A test pins the timing function for that reason; one asserting only the keyframes name would have passed a dead spinner
  - `prefers-reduced-motion: reduce` slows the animation rather than stopping it: a frozen spinner reads as a hung page, and a loading indicator has to keep indicating
  - **New Jest coverage on both paths**, and the first tests in the suite to load a real stylesheet - visibility is a pure CSS consequence, so assertions on classes alone would pass with a spinner that never appears. See `tests/js/README.md`, including the note that jsdom cascades by document order with no specificity
  - **No migration and no new upgrade script**: CSS and browser JS only. No configuration key, no database column, no template change, and no request shape changes
...
- **Reverted: the zero-cap "overcharge" guard withheld Two from every buyer on affected shops** (TWO-25276, reverting part of TWO-25269, umbrella TWO-24739)
  - **The premise of the guard was false.** TWO-25269 (entry below) added a check in `convertTwoBuyerFeeShareCurrency()` that failed closed when a **configured** cap converted to `0.00`, on the stated reasoning that "a zero cap is indistinguishable downstream from *no* cap, so passing it through sends an uncapped percentage - an overcharge". The API does not behave that way. In checkout-api, the order-fee calculation (`fees.py`) reads the cap as `c = buyer_fee_share.cap` and then applies `if c is not None: b = min(b, c)` - an `is not None` test, **not** a truthiness test - and the fee schema documents `cap: Money | None` with "None means no cap". Its fee test suite pins the outcome with a case named `cap 0 forces zero regardless of surcharge`, expecting `"0.00"`. A zero cap therefore **clamps the fee to zero**: the surcharge is simply not applied, which is exactly what capping at zero asks for. There was never an overcharge to guard against
  ...
  - **No rounding was changed.** The pricing API rejects sub-cent values - `Money` quantizes to 2 decimal places under a `Decimal` context trapping `Inexact`/`Rounded`, so an unrounded figure is an HTTP 422, not a more precise cap. Every `round($converted, 2)` stays
