...
### Changed
- **Optional buyer reference fields moved into the Two payment tile, two more added, all four ON by default** (ABN-472, umbrella TWO-24739)
  - **BEHAVIOUR CHANGE ON UPGRADE, deliberate and accepted.** `upgrade-2.7.0.php` **writes** `PS_TWO_ENABLE_DEPARTMENT`, `PS_TWO_ENABLE_PROJECT`, `PS_TWO_ENABLE_PO_NUMBER` and `PS_TWO_ENABLE_INVOICE_EMAIL` to `1` on every existing shop, over a stored `0`. Merchants who want one of them off must switch it off after upgrading. Seeding only absent keys would have been very nearly a no-op: the admin form has always saved department and project on every submit, so practically every live shop already carries a stored `0` for both, and that `0` came from `install()` never writing a default (the switches therefore rendered off) rather than from a decision. The agreed cross-platform out-of-box state is all four visible
  ...
  - **Order comments are still core's**, deliberately: PrestaShop's own "add a comment about your order" textarea (`delivery_message`) on the checkout shipping step. No plugin order-note field was added. It is not relayed to Two - `order_note` is still sent empty
