...
- **A completed sole-trader enrolment left the address form's identification field empty** (TWO-40 follow-up)
  - The enrolled identity was persisted server-side and published in-memory, but nothing wrote the organisation number into the address form. `two:sole-trader-ready` now carries it, and the company-search control mirrors it into the address `dni` field through the existing writer - so it is marked as autofilled, `vat_number` is still never written, and no `input`/`change` is announced, which is what keeps it clear of the stale-selection clear
  - **Gated on the merchant's "Autofill company address" setting** (`PS_TWO_ADDRESS_LOOKUP`), the same control every other address write a company capture performs already obeys, reached through the same gated writer rather than any check of its own. Deliberately *not* gated on where company search is mounted: that setting happens to be forced off when search moves to the payment tile, but the two are separate questions and the coincidence is not relied on
  - A sole trader with no registered number enrols under a synthetic internal identifier, and that value is refused here: PrestaShop saves the identification field onto the address and can print it. The handler also stands down while the buyer is in manual entry, like every other write path in the control - enrolment is asynchronous and must not overwrite details being typed
  - Deliberately **no** write into the visible company field, and **no** street/postcode/city write-back. The wider adoption was attempted and withdrawn (see `.ai/decisions.md`); the buyer-autofill endpoint is consumed for the email, company name and organisation number only, on this plugin and the other platforms alike, so there is no verified address payload to write
