...
- **The upfront "Registered business" / "Sole trader" chip choice is removed; a three-chip mode selector - "Sole Trader" / "Registered Company" / "Enter Manually" - is folded directly into the company search control instead** (TWO-40, pilot for the plugin-parity rollout)
  ...
  - **A still-resolving sole-trader enrolment can no longer overwrite a REAL company the buyer subsequently searched for and selected** (adversarial review BLOCKER). Removing the previous finding's `enrolling` gate from the popup-completion listener reopened exactly the failure it fixed, from the other side: a buyer-lookup or `saveCompany` round trip that was still in flight when the buyer went back to search and picked a different, real company could resolve afterward and silently publish the sole-trader identity over it - with the order-intent credit check then running against the wrong entity and nothing on screen to show it happened. Fixed with a generation counter (`_enrollGeneration`), bumped on every `cancelEnrollment()` call (i.e. every route back into ordinary search) and checked before any buyer-lookup response touches the DOM or publishes a selection
