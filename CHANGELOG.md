...
- **The Two payment tile is now withheld from buyers outside the merchant's own per-module country allowlist, instead of failing at the last click** (TWO-25387)
  ...
  - Two further cases land here for the same reason: a merchant who switches that setting to the invoice address still gets no protection when the contextual `Country` fails to load at all — core skips its country clause entirely in that state and renders every payment module — and neither filter runs on a cart core never dispatched the hook for
  ...
  - **Fail-closed on a genuine "no such row", fail-OPEN on a lookup that could not be answered at all.** A row core cannot find here is a row it will not find at submission either. But a thrown DB error is not a restriction verdict, and treating it as one hides Two on every shop at once — which is not hypothetical: the first cut of this gate carried a redundant `LIMIT 1` (core's `Db::getValue()` appends its own, so the query was a syntax error), and under a uniformly fail-closed branch that silently removed the payment method from every shop. Only the Playwright e2e job caught it. The offline harness now reproduces that fatal, and the emitted SQL is asserted directly
