...
- **The payment tile now shows the captured company back to the buyer, read-only** (TWO-25288, umbrella TWO-24739)
  ...
  - **It re-paints after PrestaShop replaces the payment step**, which happens for something as ordinary as a voucher or a surcharge-line sync. Listeners are delegated off `document` and the values are re-read on every render, so nothing is cached from a node that is about to be replaced; the re-render is deferred a macrotask so it lands after the swap rather than during it. The company search calls in directly at each point where it changes the pair, because those writes go through `.val()` / `.attr()` and fire no event that could be observed
