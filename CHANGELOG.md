...
- **Configuration key `PS_TWO_ENABLE_COMPANY_NAME` is renamed to `PS_TWO_COMPANY_SEARCH_LOCATION`** (TWO-40)
  ...
  - Real rename, not a permanent alias: `upgrade/upgrade-2.7.5.php` carries the value across **per shop** and deletes the old row. An absent row stays absent and keeps resolving to the address-area default, rather than having a decision invented for it. A value already stored under the new name is never overwritten by the old row — **newer wins** — so a merchant who saves the position before the upgrade script runs does not have that save reverted
  - **One compatibility read remains, and expires by enforcement.** A PrestaShop upgrade script runs only when the module upgrade is actually triggered (back-office Module Manager), *not* when a deploy merely swaps the module files. In that window the new key is absent while the old row is still in the DB, which without a fallback would silently flip a tile-mode merchant back to the address area **and** re-enable address autofill on a live storefront. `Twopayment::isCompanySearchInAddressArea()` therefore still reads the old name when the new key is absent. It is scheduled for deletion in 2.8.0 by a test that turns red once the declared version reaches it — not by anyone remembering. `uninstall()` deletes both spellings for the same one release
