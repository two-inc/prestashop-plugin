...
- **The Two payment tile is now withheld from buyers outside the merchant's own per-module country allowlist, instead of failing at the last click** (TWO-25387)
  ...
  - It was only ever applied to Two at **final order submission**: the payment controller asks core whether this module is authorised, and core's answer matches the allowlist against the cart's **invoice address** country. Core's display-time filter matches the same table against the **contextual** country instead — so on any shop where those two differ (the usual case: a buyer whose billing address is not the shop's context country), the tile rendered, the buyer filled in the whole payment step, and the order was refused at the last click
  - `hookPaymentOptions()` now consults a new `checkCountry()` alongside `checkCurrency()`, and withholds the payment option with a log line when the billing country is not enabled — the same shape, and the same reasoning, as the unsupported-currency gate that already sat next to it
  ...
  - Fail-closed on an inconclusive lookup, deliberately: a row core cannot find at display time is a row it will not find at submission either. A shop that has never touched the Payment Restrictions screen is unaffected — module install writes a row per active country
