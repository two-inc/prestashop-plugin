...
- **A completed sole-trader enrolment left the address form's company field empty** (TWO-40 follow-up)
  ...
  - **Address-form placement only.** The payment-tile placement must never write into the address form, so the placement is now passed explicitly to both mounts and is the handler's first check - belt and braces alongside the tile mount's existing hardcoded `addressLookupEnabled: false`
