...
- **The company-search dropdown now offers a way out of itself** (TWO-25288, umbrella TWO-24739)
  ...
  - **Choosing it forgets the selected company, and that is load-bearing rather than tidying.** "My company is not on the list" is a statement that the selected company is *wrong*, and the server-side resolver consults the **session company first**, ahead of the address — discarding it only on a country mismatch or an address switch, never on the company name changing. So clearing the hidden organisation number alone would leave the buyer looking at an empty field while the order still carried the company they had just disowned, and placement would credit-check that company. Entering manual entry now clears the hidden number, its company-name marker, **and** the session company. The clear needed its own endpoint action: the existing save action rejects an empty company id outright, so calling it to clear would have been a silent no-op. The same hole remains reachable by retyping over a selection — that is pre-existing and tracked separately — but this element promotes it to a one-click route sitting directly under the results the buyer just chose from, so it could not ship relying on that
