...
- **JS test harness for the module's front-office JavaScript, plus company-search regression tests** (TWO-25239, umbrella TWO-24739)
  ...
  - `tests/js/` is Jest + jsdom, gated in CI as the `jest` job in `.github/workflows/tests.yml` and run locally with `make test-js`. Layout mirrors `magento-plugin`'s `Test/Js/`: a root `package.json` holding only JS devDependencies (excluded from the release zip by `package-release.sh`, as `package-lock.json` already was), a jest config beside the tests with `rootDir` back at the repo root, and `testEnvironment: 'jsdom'`. Test files glob, unlike `tests/run.php`'s explicit `require` list
  ...
  - Coverage: `responseCallback` fires **exactly once** per search on all nine paths (short term, success, timeout, network error, parser error, abort, superseded, stale-success-outrunning-its-abort, teardown mid-search); a failure reports `unavailable` and never an empty result set; abort stays silent while a timeout does not; `degraded === true` strictness including absent-means-false and every truthy non-`true` value; the `_renderItem` patch surviving 100 re-setups, 20 country changes and 20 `updatedAddressForm` events as the same function; the unavailable row being non-selectable and text-only; a destroyed instance writing nothing, with mirror-image tests proving a live instance in the same position still does; and the custom no-jQuery-UI fallback's own spinner and orphan-container behaviour
