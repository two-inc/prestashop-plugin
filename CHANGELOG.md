...
- **Clicking "I'm a sole trader" on the address-editor page dead-ended in silence** (TWO-40 follow-up, live bug on staging)
  ...
  - The invoice-address precondition is replaced by a trust-ordered country resolver: the cart's invoice address first (never overridden by anything the request carries), then a posted `country` accepted only in exactly the two-upper-case-letter ISO shape, then the cart's delivery address. Nothing resolvable is still a refusal - now worded as a country problem rather than an address one
...
- **The company-search location setting is renamed `PS_TWO_ENABLE_COMPANY_NAME` -> `PS_ENABLE_COMPANY_SEARCH_IN_ADDRESS`, migrated by `upgrade/upgrade-2.7.6.php`** (TWO-40 item #1, version 2.7.6)
  ...
  - **File-swap caveat, which the script cannot close.** Upgrade scripts only run via the back-office Module Manager or `dev/ci/upgrade-module.sh`. A deploy that merely replaces the module's files — which is how the git-synced shops update — does NOT run them. Between the file swap and someone opening the back office the new key is absent, the resolver falls back to its default, and a shop configured for tile mode silently gets the search back in the address area **and** address autofill re-enabled (`getAddressLookupEnabled()` keys off the same resolver) on a live storefront. There is deliberately **no** read shim for the old key (Doug: "not a permanent alias"), so opening the module's configuration page once after a file-swap deploy is a real release step
