...
- **A completed sole-trader enrolment left the address form's company field empty** (TWO-40 follow-up)
  - The enrolled identity was persisted server-side and published in-memory, but never written into the form - so the buyer came back from the hosted signup to a blank company field, and the address they saved carried no company. `two:sole-trader-ready` now carries the enrolled name and organisation number, and the company-search control adopts them into the address form's `company` field (marked as autofilled, so a later selection can still tell it from typed input) and its identifier field through the existing address-lookup-gated writer
  - **Address-form placement only.** The payment-tile placement must never write into the address form, so the placement is now passed explicitly to both mounts and is checked before any write - belt and braces alongside the tile mount's existing hardcoded `addressLookupEnabled: false`. The handler also stands down while the buyer is in manual entry, like every other write path in the control: enrolment is asynchronous and must not overwrite details being typed
  - **The adoption no longer voids the enrolment it just saved** (adversarial review finding). Writing the company name announces `input`, whose handler clears the whole selection - including a `clearCompany` request and a blanked in-memory selection - unless the organisation number and its name tag already agree with the name in the field. For a buyer who had confirmed a registered company earlier in the session, that meant adopting the enrolment immediately undid it. The number and its tag are now written *before* the visible name, so there is nothing stale left to clear; the confirmed selection and the session cookie are re-asserted at the end of the handler as a backstop, not as the mechanism
  - **The visible company field never receives the synthetic internal identifier** (adversarial review finding). The event's company *label* deliberately falls back to the organisation number, and for a sole trader with no trading name that number is the internal `TWO:`-prefixed identifier - which would then have been saved onto the address and printed on the invoice. The event now carries the real trading name separately, and only that value is written into the field the buyer sees; it is left untouched when there is no name. The identifier still travels wherever it legitimately has to, including the identifier field the credit decision reads
  - Deliberately **no** street/postcode/city write-back: the buyer-autofill endpoint is consumed for the email, company name and organisation number only, on this plugin and the other platforms alike, so there is no verified address payload to write
