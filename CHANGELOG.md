...
### Changed
- **The upfront "Registered business" / "Sole trader" chip choice is removed; sole-trader enrolment is folded directly into the company search control** (TWO-40, pilot for the plugin-parity rollout)
  - Company search is now the single entry point regardless of business type. There is no separate chip to pick before searching: TwoCompanySearch.js's dropdown gains an "I'm a sole trader" row, a sibling of "My company is not on the list", shown whenever the registry says the billing country supports sole traders
  - All enrolment mechanics (token minting, the hosted signup popup, autofill from `/autofill/v1/buyer/current`) are unchanged and still owned by `TwoSoleTrader.js`, now started via `startEnrollment()` rather than a chip click. The server-side availability handover (`.two-sole-trader`'s `data-two-available`/`data-two-country`) is also unchanged - it still lets the browser answer with no request on first paint, now feeding the search control's row instead of a toggle
  - Reopening company search cancels an in-progress, not-yet-completed enrolment (tokens already minted are kept, so resuming does not re-mint)
  - New translatable key `company_search_sole_trader_entry`, added to the `no`/`nl`/`sv` catalogues; `sole_trader_registered_business`/`sole_trader_label` are retired
