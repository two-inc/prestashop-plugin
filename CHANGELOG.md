...
- **The company-search spinner is the same loader GIF on every plugin, and the duplicate rule that overrode it here is gone** (TWO-25288, umbrella TWO-24739)
  ...
  - **No migration and no new upgrade script**: CSS, one binary asset and browser JS only. No configuration key, no database column, no template change, and no request shape changes
