...
**Status:** Implemented. The PR1 + PR2 scopes described below landed in `twopayment.php` under Linear TWO-24880 (prestashop-plugin PR #73, merged 2026-07-16). Still outstanding as follow-ups: the §4.3 pt 4 `NET_FORMULA_TOLERANCE` tightening (deliberately left at 0.05, see the constant's inline comment), the §9 CI compatibility matrix, the §10 end-to-end suite and the §11 cross-plugin review.
**Author:** Two Engineering
**Date:** 2026-06 (design), implemented 2026-07
...
> **Reading note.** This document describes the *pre-implementation* code in the present tense in places, because that is how it was written. Every `twopayment.php` line number quoted below is **historical** — it refers to the file as it stood before TWO-24880 and no longer resolves. Passages describing code that TWO-24880 deleted have been rewritten in the past tense; treat any remaining line reference as a pointer to search by symbol name, not as a location.
...
- **Always** source the product rate from the address-correct calculator. Do **not** use `$line_item['rate']` (country-only). **Helper signature:** the loop iterates `getProducts(true)` *arrays* — there is no `$product` object and the rows do **not** carry `id_tax_rules_group`. Resolve the group id explicitly per line via `Product::getIdTaxRulesGroupByIdProduct((int)$line_item['id_product'], $this->context)` (second arg is a **`Context`**, not a `Shop` — the method reads `$context->shop->id` internally to scope the lookup; passing a `Shop` trips the type hint. The group is product+shop-scoped, not per-combination; combinations carry price/ecotax but not their own tax group). Then `getTwoConfiguredTaxRateDecimalForGroup($taxRulesGroupId, $cart)`.
- **Never** `tax÷net`.
...
| Shipping line | carrier group **via the shared helper** (not a snap candidate); where no carrier group is resolvable, the optional merchant-declared default of §4.2.4, else refuse | `getPackageShippingCost()` (real cost pre-free-ship) |
...
**4.2.1 Free-ship `min()` cap fix (implemented):** the free-ship builder *used to* cap `alloc_net = min(alloc_net, discountNetTotal, alloc_gross)` *independently* of `alloc_gross`, so when the cap bit (any normal multi-discount cart) `alloc_tax = alloc_gross − alloc_net` no longer equalled `rate·alloc_net` → the §4.3 assertion would throw on a **legitimate** cart. Fixed under TWO-24880: when the cap bites, `alloc_gross` is re-derived as `alloc_net + round(alloc_net·rate, 2)` so gross/net/tax stay rate-consistent — net is never clamped independently of gross.
...
**4.2.4 Default shipping tax code — a second place for the merchant to DECLARE, not a place for us to infer (IMPLEMENTED, TWO-25200).** PrestaShop keeps the shipping tax-rules group on the carrier row (`carrier_tax_rules_group_shop`) and nowhere else; there is no shop-level shipping group (core defines only `PS_ECOTAX_TAX_RULES_GROUP(_ID)` and `PS_GIFT_WRAPPING_TAX_RULES_GROUP`). A merchant pricing shipping outside the carrier table entirely — custom logistics, `id_carrier = 0` — therefore has **nowhere in core to declare shipping VAT**, and `getDeliveryOptionList()` discards the whole option list on that sentinel (8.1.7:2909, 9.0.0:2674), so §4.2's carrier lookup has nothing to read. Pre-TWO-25200 that was a flat refusal, correct under §5 but leaving such a merchant with no route to a correct order at all.

The resolution is an **optional module-level config key** (`PS_TWO_DEFAULT_SHIPPING_TAX_RULES_GROUP`) holding a `TaxRulesGroup` the merchant picks. This does **not** weaken §2 or §5: it is still the merchant's own declaration resolved through the shared §4.1 helper, not a rate inferred from amounts, and not one of the inference candidates §4.2 rules out (`getIdTaxRulesGroupMostUsed()`, `PS_CARRIER_DEFAULT`, `tax ÷ net`). It only moves *where* the declaration lives when core offers no row to put it on.

- **Order:** carrier group → this default → the existing loud refusal. The default is consulted *only* on the path that would otherwise have thrown, so a shop with a working carrier table can never reach it, and the per-option/per-address split of §4.2 is never degraded by it.
- **No default value, ever.** Unset (the shipped state, and what `install()` leaves behind — `upgrade-2.6.8.php` deliberately seeds nothing) is byte-for-byte the pre-TWO-25200 refusal. Seeding anything, "No tax" included, would relay a rate the merchant never declared.
- A selection pointing at a since-deleted group is **not** a declaration: treated as unset, so the order is refused rather than relayed at 0%.
- Whole shipping charge goes into **one** rate class. The carrier-level split it substitutes for is unavailable by construction on this path, and the declaration is about shipping as such.
- **Hidden unless the install opts in** with `define('_TWO_ENABLE_DEFAULT_SHIPPING_TAX_CODE_', true);` in `config/defines_custom.inc.php` (core's sanctioned per-install override, `include_once`d by `config/config.inc.php` on every front/admin/CLI request before modules load, preserved across core upgrades, FTP-editable on shared hosting). The constant gates **field visibility only** — a stored value survives the constant disappearing, and a save while the field is hidden never rewrites the key (the `TWO-24813` wipe-on-hidden-field class of bug). Auto-detecting `id_carrier = 0` was rejected: it cannot be established without an existing order and would not notice a later carrier-setup change.
- **Observability is part of the design:** using the fallback logs at severity 2 naming the group, its id and the resolved rate, so "this shop is on the fallback" is a log grep rather than an inference. The refusal log correspondingly drops from severity 3 to 2 when a default is configured — the refusal is then an internal control-flow step, not a failure.

### 4.3 Divergence handling — plugin-side, deterministic
A per-line check already existed (`validateTwoLineItems`, throwing at its call site) — but it **logged + returned bool** with a bare `tax÷net` tolerance check, a looser `NET_FORMULA_TOLERANCE` net check, and **no gross check**. So this was a **rewrite of the validator body, not a "repurpose"** — the merge gate had to exercise the new branches. It was rewritten under TWO-24880 so the plugin gate matches the Two API's reconciliation semantics (else we false-decline carts the API accepts, or pass carts it rejects). Points 1–3 below are implemented; point 4 is **not** — see the note under it:
...
4. **Tighten the net check (`NET_FORMULA_TOLERANCE`) to the API's line tolerance.** **STILL OPEN** — the constant remains at `0.05` deliberately, because the emitted `unit_price` is 2dp while the discount is derived at 6dp, so a legitimate high-quantity line can drift up to `qty·0.005`. Tighten only after that absorption gap is fixed. The looser value buys nothing — the API rejects the gap when enforced, and per the fail-loud principle a mismatch between emitted `qty·unit_price − discount` and the authoritative net is a divergence we want surfaced. Confirm the discount-absorption mechanism keeps legitimate **high-quantity** lines within the tightened tolerance (high-qty fixture) before tightening; if a real class needs more headroom, that's an absorption gap to fix.

**Plugin aggregate note:** the plugin's *order-level* reconcile (`isTwoAmountWithinTolerance`) is tolerant, not exact. The exact guarantee we add is **per-line** (`gross==net+tax`, point 3).

On breach, **throw** (mirrors the pre-existing negative-discount `throw` in the discount-precision path), making case 3 fail loud **regardless** of whether the API's strict reconciliation is enabled for the merchant. We do **not** silently switch to the amounts-rate or snap — that would be correcting the merchant's declaration. The API is the backstop.

**Blast radius (verified safe):** all three throw-reachable call sites — approval precheck, live submit (`payment.php`), post-payment confirmation (`confirmation.php`) — wrap the builder in try/catch and degrade to a controlled decline (no white-screen / unhandled 500). The rich exception message goes to `PrestaShopLogger` (for the merchant); the buyer sees a generic "review your cart" notice. So the actionable detail is a merchant/logs artefact, not buyer-facing.
...
**Keep** (the core of the prior work carries straight over — see §1's note on what it got right): product iteration + amounts; unit-price/discount precision (native 6dp, raises on negative discount); ecotax decomposition (`3411640`); shipping real-cost read + carrier-rate lookup (`0fa488b` — already the authoritative-source approach this design generalises); gift-wrapping-as-own-line (`fcbc793`); `buildTwoFallbackFreeShippingDiscountLine` (its `tax÷net`+snap was deleted; the rate now mirrors the shipping line); `buildTwoCanonicalDiscountRateSegments`/`solveTwoRateDiscountSplitInCents` (the declared-rate path for discounts — kept; their `tax÷net` fallback arms were deleted and the solver raises on unsolvable). Net: most of the existing structure stayed; the change was making rate-sourcing uniform, not rebuilding the builder.

**Delete (sequenced) — all of the below was removed by TWO-24880 (prestashop-plugin PR #73); none of these symbols exist in `twopayment.php` any more:**
- The divide-then-snap logic at all five sites (product, shipping, wrapping, discount arms, Spanish canonical-rate fallback). **Removed.**
- `snapTwoTaxRateToKnownContexts` + `TAX_RATE_VARIANCE_TOLERANCE`/`TAX_RATE_CONTEXT_SNAP_TOLERANCE`: had 6 callers — 3 in PR1 sites, 3 in PR2 discount arms. Helper and both constants were **deleted** once the last caller went.
- `SPANISH_FALLBACK_TAX_RATE` and its consumers — `applyTwoSpanishCanonicalTaxRateFallbackToItems` (which post-processed the **full** items array incl. discount lines, guarded by `shouldApplyTwoSpanishTaxRatePolicy`), `shouldApplyTwoSpanishTaxRatePolicy`, `collectTwoKnownTaxRatesFromConfiguredProductRates` and `collectTwoKnownTaxRatesFromPositiveItems` — were all **deleted**. The planned two-step (PR1 forces the existing gate false, PR2 deletes) was collapsed into the single TWO-24880 change.
- `normalizeTwoTaxRateToPercentPrecision` was **kept** (it enforces the precision ceiling) and now runs on the declared rate (already canonical), after sourcing and before format.

### Priority: replace the canonical-rate fallback early — done
`applyTwoSpanishCanonicalTaxRateFallbackToItems` **used to** resolve a line's rate to the nearest canonical value when the derived rate drifted. That is a side effect inherent to any snap-to-canonical step: in a mixed-rate edge case it could resolve a line to a neighbouring canonical rate (e.g. a 10% line landing on 21%) while still satisfying the amount checks, so the divergence was never surfaced — which on an ES invoice would be an incorrect VAT breakdown. The declared-rate approach removes this class of edge entirely. The fallback was **removed outright under Linear TWO-24880 (prestashop-plugin PR #73, merged 2026-07-16)**, rather than gated off in PR1 and deleted in PR2 as sequenced above.
...
**Bounding false positives:** the binding intent gate is the per-line tolerance, not a per-group exact check, so many-small-cheap-line carts do not aggregate into a reject. The one real per-line risk is `PS_ROUND_TYPE = ROUND_ITEM` + high quantity — covered by a §7 fixture. The wrapping `gross<net→gross=net` clamp silently masked divergence; it was **removed under TWO-24880** and the wrapping builder now logs and fails loud when wrapping gross diverges below net.

## 6. Sequencing — two PRs

**Outcome:** the two-PR split below was **not** followed. Both scopes shipped together in a single change under Linear TWO-24880 (prestashop-plugin PR #73, merged 2026-07-16), so the interim "gate it off in PR1, delete in PR2" steps never applied — the derive/snap/Spanish-fallback helpers were deleted outright. The §9 CI compatibility matrix was deliberately split out to a follow-up. The list is retained as the record of what the change had to cover.
...
- Fix the ecotax rate source: `extractTwoEcotaxLineBreakdown` fell back to the country-only `$line_item['rate']`; it now resolves the rate through the shared helper with `PS_ECOTAX_TAX_RULES_GROUP_ID` (matching the rate PrestaShop embedded in `total_ecotax`). **Done.**
...
6. **`reduction_tax == 0` semantics** — fixture-verify (PrestaShop: 1=tax-incl, 0=tax-excl).
...
2. **PHP constraint per PrestaShop version** — fetch that tag's raw `composer.json` from GitHub and parse the `php` constraint in the `require` block (the `.require.php` key — not a file named `require.php`; e.g. `>=8.1` → minors `[8.1, 8.2, …]`), exactly as the Magento script parses Magento's constraint.
