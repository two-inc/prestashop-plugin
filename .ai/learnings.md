...
**Fifth lesson, and it supersedes the "Fix" line above (round 4)**: the first-paint guard was the wrong fix, and it was wrong in a way the measurements could not show. It was head-loaded correctly (verified on the delivered checkout HTML: it is the ONLY external `<script>` before `</head>`, at byte 3313 of 66107, with `</head>` at 30603), and it did suppress the beat it targeted - but it targeted an ARTEFACT of the reload and left the reload. So it could never touch the other half of the report, the whole tile being destroyed and rebuilt on SELECTING Two, where there is no first paint to suppress. Read core rather than only instrumenting the symptom: `themes/_core/js/cart.js` -> `refreshCheckoutPage()` (`themes/_core/js/common.js`) is `window.location.href = pathname + '?updatedTransaction=1'`, then `window.location.reload()` thereafter, and core's stated reason is to let other payment modules recompute the amount - a reason this module does not need, because its fee line only exists while Two is selected and is stripped server-side before any other payment module's controller runs. The fix is to stop navigating: POST the same `.js-cart` refresh URL core would and swap the same summary partials in place. Removing the cause deleted the guard, its stylesheet rule, its sessionStorage restore protocol and its test file.
**Sixth lesson (process)**: two consecutive sub-agents tasked with the live browser measurement returned nothing - one denied mid-brief, one stuck waiting on its own spawned monitor. Plain `curl` with a cookie jar answered the load-order question in three calls, and core's own published source answered the rest. Reach for the browser when you need *timing*; reach for the delivered bytes when you need *structure*.
**Files**: `views/js/modules/TwoSoleTrader.js`, `views/js/modules/TwoCheckoutManager.js`, `views/templates/hook/paymentinfo.tpl`, `twopayment.php`, `classes/TwoSoleTrader.php`, `views/css/two.css`
